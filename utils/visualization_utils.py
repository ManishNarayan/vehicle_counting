...
#vcap = cv2.VideoCapture("video_1.mp4")
#height = int( vcap.get(cv2.CAP_PROP_FRAME_HEIGHT ))
ROI_POSITION = 390#int((height)/3)
#vcap.release()
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,image,
  ...
  for box, color in box_to_color_map.items():
    ...
    if (("truck" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image_array(current_frame_number,
                image,
                ymin,
                xmin,
    ...
    if (("bus" in display_str_list[0])):
            ...
            if(1 in is_vehicle_detected):
              ...
              csv_line_util = class_name + "," + csv_line
    if (("person" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image_array(current_frame_number,
                image,
                ymin,
                xmin,
            ...
            if(1 in is_vehicle_detected):
              ...
              csv_line_util = class_name + "," + csv_line
    if (("motorcycle" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image_array(current_frame_number,
                image,
                ymin,
                xmin,
